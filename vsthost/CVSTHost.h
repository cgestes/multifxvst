...
public:
    ...
    CFxBank(int nPrograms, int nParams,bool IsPreset = false);
    CFxBank(int nChunkSize,bool IsPreset = false);
    CFxBank(CFxBank const &org) { DoCopy(org); }
	virtual ~CFxBank();
    CFxBank & operator=(CFxBank const &org) { return DoCopy(org); }
public:
    //la banque est un preset ac des params
    bool SetSize_Preset(int nParams); 
    //la banque est une banque ac des params
    bool SetSize(int nPrograms, int nParams);
    //la banque est un preset chunk
    bool SetSize_ChunkPreset(int nChunkSize);
    //la banque est une banque chunk
    bool SetSize_Chunk(int nChunkSize); //preset = no
	bool LoadBank(char *pszFile);
	bool SaveBank(char *pszFile);
	void Unload();
    bool IsLoaded() { return !!bBank; }
    bool IsChunk() { return bChunk; }
    bool IsPreset() {return bPreset;}
...
public:
    ...
    void SetFxVersion(long v) { if (bBank) ((fxSet*)bBank)->fxVersion = v; if (!bChunk && !bPreset) for (int i = GetNumPrograms() -1; i >= 0; i--) GetProgram(i)->fxVersion = v; }
    ...
    long GetChunkSize() { if (!bChunk) return 0;
                 if(bPreset)
                   return *((int *)((fxProgram *)bBank)->params); //vachement mieux!
                   //return ((fxProgram *)bBank)->byteSize - sizeof(fxProgram)  + 8  ;
                 else
    ...
    { if (!bChunk) return 0;
        if (bPreset)
          return ((fxProgram *)bBank)->params+1;  //soit a la sauvage :bBank+60
        else
         return ((fxChunkSet *)bBank)->chunk;
    }
    bool SetChunk(void *chunk)
    { if (!bChunk) return false;
       memcpy(GetChunk(), chunk, GetChunkSize()); 
       return true;
    }
...
{
public:
    CFxPreset(char *pszFile = 0);
    CFxPreset(int nPrograms, int nParams);
    CFxPreset(int nChunkSize);
    CFxPreset(CFxPreset const &org) { DoCopy(org); }
	virtual ~CFxPreset();
    CFxPreset & operator=(CFxPreset const &org) { return DoCopy(org); }
public:
    bool SetSize(int nPrograms, int nParams);
...
public:
    ...
    CEffect *GetAt(int nIndex){ 
      if ((nIndex >= 0) && (nIndex < naEffects))
        return (CEffect *)aEffects[nIndex];
      else return 0; }
