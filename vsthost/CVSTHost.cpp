...
CFxBank::CFxBank(int nPrograms, int nParams, bool IsPreset)
{
Init();                             
if(IsPreset)
  SetSize_Preset(nParams); 
else
  SetSize(nPrograms, nParams); 
}

CFxBank::CFxBank(int nChunkSize,bool IsPreset)
{
Init();
if(IsPreset)                             
  SetSize_ChunkPreset(nChunkSize);     
else
  SetSize_Chunk(nChunkSize);    
}
...
memset(nBank, 0, nTotLen);              /* initialize new bank               */
  ...
  fxProgram *pSet = (fxProgram *)bBank;
  pSet->fxMagic = fMagic;
  pSet->chunkMagic = cMagic;
  pSet->byteSize = 0;
  pSet->version = MyVersion;
  pSet->numParams = nParams;
...
/*****************************************************************************/
/* SetSize : sets new size     of a bank                                     */
/*****************************************************************************/
...
int nProgLen = 0;
 ...
 nTotLen = sizeof(fxSet) - sizeof(fxProgram);
 nProgLen = sizeof(fxProgram) + (nParams - 1) * sizeof(float);
...
bool CFxBank::SetSize_Chunk(int nChunkSize)
{
  //CTAFINSCONSCIENCE (ou ctaf conscience va savoir!)
...
bool CFxBank::SetSize_ChunkPreset(int nChunkSize)
{

int nTotLen = sizeof(fxProgram) + nChunkSize /*+ 8*/;
unsigned char *nBank = new unsigned char[nTotLen];
if (!nBank)
  return false;
...
pSet->chunkMagic  = cMagic;
pSet->byteSize    = nTotLen - 8;
pSet->fxMagic     = chunkPresetMagic;
pSet->version     = MyVersion;
pSet->numParams   = 0;//num param  (cubase le met)/*numPrograms = 1;*/
*((int *)pSet->params)   = nChunkSize; //chunksize
...
try
  ...
  if (pSet->fxMagic == fMagic) //fxp preset
  {
    bPreset = true;
    bChunk  = false;
    fxProgram * pProg = (fxProgram *) pSet; 
    for (int j = 0; j < pProg->numParams; j++)
      SwapBytes(pProg->params[j]);
    //juste verif taille du prog + param = taille du prog + numparam * taille(flaot)
  }
  else if (pSet->fxMagic == bankMagic)
    {
    ...
    if (NeedsBSwap)                     /* eventually swap necessary bytes   */
      ...
      if (pCSet->chunkSize + sizeof(fxChunkSet) - 8 > tLen)
        throw (int)1;
      }
    }
    else if ( pSet->fxMagic == chunkPresetMagic)
    {

    fxProgram  * pCSet = (fxProgram *)nBank;
    if (NeedsBSwap)                     /* eventually swap necessary bytes   */
      {

      SwapBytes(pCSet->params[0]);
      unsigned int nb = *((int *)pCSet->params);
      //verification pourri faudrait verif (pCSet->param)
      //if (unsigned(pCSet->byteSize + 8) > tLen)
      if (nb + sizeof(fxProgram) > tLen)
        throw (int)1;
  ...
  nBankLen = (int)tLen;
  bChunk = (pSet->fxMagic == chunkBankMagic) || (pSet->fxMagic == chunkPresetMagic);
  bPreset = (pSet->fxMagic == chunkPresetMagic) || (pSet->fxMagic == fMagic);
  }
catch(...)
  {
...
if (bChunk)
  ...
  if (NeedsBSwap)                       /* if byte-swapping needed           */
    if(bPreset)
      SwapBytes(((fxProgram *)pCSet)->params[0]);
    else
      SwapBytes(pCSet->chunkSize);
...
else
  {
    if(bPreset)
    {
      fxProgram * pProg = (fxProgram *)nBank;
      int numParams = numPrograms;
...
{
if ((!IsLoaded()) || (bChunk && !bPreset))          /* if nothing loaded or chunk file   */
...
//comme ca mm si le plug nous demande avant qu'il soit enregistré
//dans l'host il redemandera tant que ca sera pas good
...
long VSTCALLBACK CVSTHost::AudioMasterCallback
    (AEffect *effect,long opcode,long index,long value,void *ptr,float opt)
{
if (!pHost)
  return 0;
/*if(!effect)
...
switch (opcode)
  ...
  case audioMasterGetVendorString :
    return OnGetVendorString(nEffect,(char *)ptr);
  case audioMasterGetProductString :
    return OnGetProductString(nEffect,(char *)ptr);
  case audioMasterGetVendorVersion :
    return OnGetHostVendorVersion(nEffect);
...
if (!pEffect)
  return;
//ASSERT(pEffect->EffCanDo("process"));
