// GratomaticDlg.cpp : fichier d'implémentation
//
...
// Boîte de dialogue CGratomaticDlg

IMPLEMENT_DYNAMIC(CGratomaticDlg, CDialog);

CGratomaticDlg::CGratomaticDlg( CWnd* pParent)
:CDialog(CGratomaticDlg::IDD, pParent)
, m_copied(-1)
, m_used(0)
{

  APP = NULL;
  TRACE("CREATE::CGratomaticDlg\n");
}

void CGratomaticDlg::OnUpdate(int nb)
{
...
void CGratomaticDlg::SetAPP(CAppPointer * m_APP)
{
  APP = m_APP;
}


CGratomaticDlg::~CGratomaticDlg()
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
  CString buf;buf.Format("DESTROY :: CGratomaticDlg(%d) \n", this);  TRACE(buf);
}



void CGratomaticDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGratomaticDlg, CDialog)
...
// Gestionnaires de messages CGratomaticDlg

void CGratomaticDlg::PostNcDestroy()
{
	CDialog::PostNcDestroy();
  APP->pGratomatic  = NULL;
	delete this;
  CString buf;buf.Format("PostNcDestroy :: CGratomaticDlg(%d) \n", this);  TRACE(buf);
}

//ajoute un plug-ins a la fin de current_chaine
void CGratomaticDlg::OnBnClickedBtnadd()
{
CString sStartAt;
...
//charge un plug-ins
void CGratomaticDlg::LoadEffect(LPCSTR dllpath)
{
  AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
BOOL CGratomaticDlg::OnInitDialog()
{
  CDialog::OnInitDialog();

  CString buf;
  buf.Format("INITDIALOG :: CGratomaticDlg(%d) \n", this);
  TRACE(buf);
...
void CGratomaticDlg::InitialiseSkin()
{
...
void CGratomaticDlg::OnBnClickedBtnup()
{
...
void CGratomaticDlg::OnBnClickedBtndown()
{
  int n = m_listvst.GetCurSel();
  if(n<0)return ;
...
void CGratomaticDlg::OnBnClickedBtnsave()
{
  CString sStartAt;
...
BOOL CGratomaticDlg::SaveAll(LPCSTR Path)
{
  TRY{
...
BOOL CGratomaticDlg::LoadAll(LPCSTR Path)
{
  TRY{
...
BOOL CGratomaticDlg::SaveChaine(int chaine,LPCSTR Path)
{
  TRY{
  CFile f(Path,CFile::modeWrite | CFile::modeCreate); 
  CArchive ar(&f,CArchive::store);
...
BOOL CGratomaticDlg::LoadChaine(int chaine,LPCSTR Path)
{
  TRY{
  CFile f(Path,CFile::modeRead); 
  CArchive ar(&f,CArchive::load);
...
BOOL CGratomaticDlg::SaveParamEffect(int chaine,int nbeffect,LPCSTR Path)
{
  return FALSE;
}

BOOL CGratomaticDlg::LoadParamEffect(int chaine,int nbeffect,LPCSTR Path)
{
  return FALSE;
}

void CGratomaticDlg::OnBnClickedBtnload()
{
CString sStartAt;
sStartAt = AfxGetApp()->GetProfileString("Load", "PathLoadAll");
if (sStartAt.GetLength())
...
void CGratomaticDlg::OnBnClickedBtnsuppr()
{
...
void CGratomaticDlg::OnBnClickedBtnsave2()
{
CString sStartAt;
...
/*void CGratomaticDlg::SetNewNumChaine(int newchaine)
{
  if(APP->current_chaine != newchaine)
     this->newchaine = newchaine;
};*/
    
//void CGratomaticDlg::OnUpdateCh()
//{
  //APP->ChangeChaine(APP->current_chaine,FALSE);
//}


void CGratomaticDlg::OnBnClickedBtnload2()
{
CString sStartAt;
sStartAt = AfxGetApp()->GetProfileString("Load", "PathLoadChaine");
if (sStartAt.GetLength())
...
HBRUSH CGratomaticDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
/*
void CGratomaticDlg::OnLButtonDown(UINT nFlags, ::CPoint point) 
{
...
void CGratomaticDlg::OnLButtonUp(UINT nFlags, ::CPoint point) 
{
...
/*void CGratomaticDlg::OnMouseMove(UINT nFlags, ::CPoint point) 
{
...
//void CGratomaticDlg::OnLvnOdstatechangedListvst(NMHDR *pNMHDR, LRESULT *pResult)
//{
...
void CGratomaticDlg::OnLvnItemchangedListvst(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CGratomaticDlg::OnNMDblclkListvst(NMHDR *pNMHDR, LRESULT *pResult)
{
  int n = m_listvst.GetCurSel();
  if(n<0)return ;
...
void CGratomaticDlg::OnBnClickedBtnclrchain()
{
  APP->chaine_eff->RemoveAt(APP->current_chaine);
  OnUpdate();
}

void CGratomaticDlg::OnBnClickedBtncopieto()
{
...
void CGratomaticDlg::OnBnClickedBtnpasteto()
{
