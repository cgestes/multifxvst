...
void CSkinHeaderCtrl::OnPaint() 
{
	...
	memDC.StretchBlt(rect.left+2, 0, rect.Width(), 16, &bitmapDC, 0, 0, 1, 16, SRCCOPY);
	...
	for(int i = 0; i <nItems; i++)
	{
		...
		if(hditem1.iOrder==0)
		{
			...
			memDC.BitBlt(rect.left,rect.top,2,16,&bitmapDC,0,0,SRCCOPY);
		}
		else
		{
			memDC.BitBlt(rect.left-1,rect.top,2,16,&bitmapDC,0,0,SRCCOPY);
			pOldBitmap = bitmapDC.SelectObject(&bitmap2);
			memDC.BitBlt(rect.left+1,rect.top,1,16,&bitmapDC,0,0,SRCCOPY);
		}
		...
		memDC.StretchBlt(rect.left+2, 0, nWidth, 1, &bitmapDC, 0,0, 1, 16, SRCCOPY);
		...
		memDC.BitBlt((rect.right-2), 0, 2, 16, &bitmapDC,0,0,SRCCOPY);
